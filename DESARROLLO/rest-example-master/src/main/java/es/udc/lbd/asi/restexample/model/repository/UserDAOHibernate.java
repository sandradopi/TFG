...
public class UserDAOHibernate extends GenericDAOHibernate implements UserDAO {
	...
	public List<Game> findByCreatorResultado(String login) {
		 return   getSession().createQuery("select g from Game g inner join g.creator c where ((c.login = :login) AND ((g.date < current_date) OR(g.date = current_date AND g.timeEnd < current_time)) AND (g.result =null))").setParameter("login", login).list();
	}
	
	@Override
	public List<Game> findAllGamesPlayed(String login) {
		return getSession().createQuery("select g from Player p inner join p.game g inner join p.player u where u.login=:login AND ((g.date < current_date) OR (g.date = current_date AND g.timeEnd <= current_time))").setParameter("login", login).list();
	}
	
	@Override
	public List<Game> findAllGamesPlayedValoration(String login) {
		return getSession().createQuery("select g from Player p inner join p.game g inner join p.player u where (u.login=:login) AND (p.valorationGame= null) AND ((g.date < current_date) OR (g.date = current_date AND g.timeEnd <= current_time))").setParameter("login", login).list();
	}
	
	@Override
	public List<Game> findRecomendadosPlayers(String login) {
		return getSession().createQuery("select g from Player p inner join p.player u inner join p.game g where ((u.login= :login) AND ((g.date > current_date) OR (g.date = current_date AND g.timeStart >= current_time)))").setParameter("login", login).list();
	}
}
